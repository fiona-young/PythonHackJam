...
class Customer:
    def __init__(self, str_in, truth_table):
        ...
        for i in range(0,self.liked_milkshakes):
            if input_list[2*i+2]:
                self.milkshake_malted = input_list[(2*i)+1]
            self.milkshake_both[input_list[2*i+1]-1]=input_list[2*i+2]
            truth_table[input_list[2*i+1]-1]=input_list[2*i+2]
...
class Milkshakes:
    ...
    def __init__(self, file_pointer: open):
        ...
        self.truth_table=[[None for i in range(0,self.number_of_flavours)].copy() for j in range(0,self.number_of_customers)]
        self.customer = []
